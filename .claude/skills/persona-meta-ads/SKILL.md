...
**Nota 23 jul — esta sesión es local (en el Mac de Ramón, vía extensión de Chrome), no una sesión en la nube.** No puede ser despertada por un Routine/cron externo (el de Claude Code Remote): se probó resumirla en modo headless (`claude -p --resume`), y la conexión al navegador se cae — la extensión necesita una ventana de Chrome abierta e interactiva, no sobrevive un proceso que se reinicia.

**Corrección 23 jul (mismo día) — sí funciona con `CronCreate`:** a diferencia de `-p --resume` (que reinicia el proceso), `CronCreate` programa el siguiente disparo *dentro de esta misma sesión, sin reiniciarla* — como el proceso nunca se corta, la conexión de Chrome no se pierde. **Confirmado funcionando en esta sesión.** Condiciones reales: la ventana/terminal debe quedar abierta y la sesión no cerrarse; el job expira solo a los 7 días, así que hay que reprogramarlo al final de cada corrida; y si en algún momento Chrome aparece desconectado, avisar a Ramón para que reconecte antes de asumir cualquier dato.
