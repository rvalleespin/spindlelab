# Publicar — Los tres mitos de la Ley 21.719

**Estado: ⏳ PENDIENTE DE PASE HUMANO DE RAMÓN. No publicado. No enviado.**

**Canal:** página de LinkedIn de SpindleLab (cuerpo, registro plural) **+** perfil personal de
Ramón, que **comparte** el post de la página con su comentario en registro singular.
**Pieza visual:** ninguna. Texto puro, como el post de página del 21-sep.
**Escribe:** esta sesión · **Aprueba:** Ramón

---

## Cuerpo del post — página de LinkedIn (copiar y pegar)

Leímos la Ley 21.719 completa, en el texto del Diario Oficial del 13 de diciembre de 2024.

Hay tres cosas que se están vendiendo como obligaciones de esa ley y que la ley no dice.

La primera es que tienes que nombrar un delegado de protección de datos. El artículo 50 dice que el responsable de datos podrá designar uno, y el artículo 49 dice que el programa de cumplimiento donde ese cargo aparece se adopta voluntariamente. Obligatorio es el artículo 48, que te pide tomar acciones para prevenir infracciones. Nombrar a alguien es una manera de hacerlo, no el requisito.

La segunda es que tienes 72 horas para avisar una filtración de datos. Ese plazo es del reglamento europeo. En el texto de la ley chilena el número no aparece. El artículo 14 sexies pide reportar a la Agencia por los medios más expeditos posibles y sin dilaciones indebidas, que es un estándar de conducta y no un reloj. Te obliga a moverte apenas te enteras, y tampoco te da tres días de colchón.

La tercera es que una pyme arriesga el 4% de sus ingresos. Ese 4% está en el artículo 35 con dos condiciones que van juntas. Se aplica a empresas que no son de menor tamaño según la ley 20.416, y solo cuando reinciden en una infracción grave o gravísima. Reincidir, dice el artículo 36, es haber sido sancionado dos o más veces en treinta meses. Una pyme en su primera infracción no cumple ninguna de las dos.

Las tres existen en el reglamento europeo. La ley chilena se le parece en varias cosas, y el atajo fue suponer que se le parecía en todas.

Si te llega una propuesta de cumplimiento, sirve preguntar de qué artículo sale cada obligación que te están cobrando. El número se puede ir a mirar.

Dejamos en el primer comentario el chequeo gratis, para la parte que sí se ve en tu sitio.

---

## Primer comentario (copiar y pegar, AL TIRO)

Acá está: https://verifica.spindlelab.cl

Se llama Verifica y Cumple. Escribes tu dominio y te muestra qué señales de la ley están publicadas en tu sitio y cuáles faltan. Gratis, sin registro, y no guarda el dominio que revisas. Lo que no alcanza a ver lo dice, en vez de darlo por bueno.

Y el texto completo de la ley está acá, por si quieres ir a los artículos: https://www.diariooficial.interior.gob.cl/publicaciones/2024/12/13/44023/01/2583630.pdf

---

## Al compartir desde el perfil personal (registro singular, copiar y pegar)

Las 72 horas las tenía yo también en mis notas, copiadas de un resumen de terceros. Las di por buenas sin ir al texto.

Se me cayeron cuando fui a buscar el artículo para citarlo y no estaba.

Por eso el post lleva el número de cada artículo, para que no tengas que creerme.

---

## Variante sin primer comentario (por si LinkedIn falla al publicarlo)

El 22-sep las escrituras de la página fallaron y el post quedó 40 minutos prometiendo un link
que no existía. Si vuelve a pasar, va esta versión: **idéntica salvo los dos últimos párrafos.**

> [...igual hasta "El número se puede ir a mirar."]
>
> Para la parte que sí se ve en tu sitio hicimos un chequeo gratis, sin registro: https://verifica.spindlelab.cl
>
> El texto completo de la ley está en el Diario Oficial del 13 de diciembre de 2024, por si quieres ir a los artículos.

---

## Trazabilidad de cada afirmación legal

**Fuente: texto oficial de la Ley 21.719, Diario Oficial N° 44.023 del 13-dic-2024, CVE 2583630
(34 páginas).** Descargado y verificado el 23-sep-2026.
Copia local: `do_2583630.pdf` · texto extraído: `do.txt` · contraste: `ley-21719.xml` (BCN).

| Afirmación del post | Artículo | Texto oficial |
|---|---|---|
| El delegado es voluntario | **Art. 50** | "El responsable de datos **podrá designar** un delegado de protección de datos personales." |
| El programa donde vive el cargo es voluntario | **Art. 49** | "Los responsables de datos **podrán voluntariamente** adoptar un modelo de prevención de infracciones consistente en un programa de cumplimiento." (el delegado es su letra a) |
| Lo que sí es obligatorio | **Art. 48** | "Los responsables de datos [...] **deberán adoptar acciones destinadas a prevenir** la comisión de las infracciones establecidas en los artículos 34 bis, 34 ter y 34 quáter." |
| No hay plazo de 72 horas | **Art. 14 sexies** | "El responsable deberá reportar a la Agencia, **por los medios más expeditos posibles y sin dilaciones indebidas**, las vulneraciones a las medidas de seguridad..." |
| El número 72 no está en la ley | — | `grep "72 horas\|setenta y dos"` sobre el texto oficial completo: **cero coincidencias**. |
| El 4% tiene dos condiciones | **Art. 35, inciso cuarto** | "En caso de que el infractor corresponda a una empresa **distinta** de aquéllas definidas como empresas de menor tamaño en el artículo segundo de la ley N° 20.416, que **reincida** en infracción de carácter grave o gravísima [...] la multa podrá alcanzar [...] al 2% o 4% de los ingresos anuales..." |
| Qué es reincidir | **Art. 36, letra a)** | "Existe reincidencia cuando el responsable ha sido sancionado en **dos o más ocasiones, en los últimos treinta meses**, por infracción a esta ley." |
| Escala general de multas (no va en el post, es el contexto) | **Art. 35** | leves: amonestación escrita o hasta 5.000 UTM · graves: hasta 10.000 UTM · gravísimas: hasta 20.000 UTM |

**Datos del producto cotejados el 23-sep-2026 contra `https://verifica.spindlelab.cl` en vivo:**
gratis y sin registro · no guarda el dominio · declina en vez de puntuar cuando no puede ver ·
Kit de implementación desde $149.000 · Revisión trimestral desde $39.000, mínimo 2 trimestres.
(Ningún precio va en el cuerpo de este post.)

---

## Por qué está escrito así

**El post no nombra a nadie y no muestra capturas.** Habla de lo que circula, no de quién lo
dice. La forma "se está vendiendo como obligación" deja el hecho en pie sin convertirlo en pelea,
y es la misma jugada del post de GEO del 21-sep, que fue el de mejor alcance del lote de
septiembre sin atacar a nadie.

**Cada desmentido trae el artículo.** Es lo que hace que la pieza no sea otra opinión más en un
rubro donde todos afirman. Con estudios de abogados como primer vertical, el número del artículo
es lo que convierte un post de marketing en algo que el lector puede ir a verificar en treinta
segundos. Por eso el link al Diario Oficial va en el primer comentario: se entrega la fuente, no
solo la conclusión.

**Cada "no" viene con su "sí".** Los tres mitos podrían haberse desmentido a secas, pero eso deja
al lector sin saber qué hacer. Cada uno cierra con lo que la ley sí pide (art. 48 para el
delegado, el estándar del 14 sexies para el reporte, la escala real del 35 para las multas), en
palabras llanas y sin jerga.

**Sin cifras propias, sin puntajes, sin clientes.** El 36 → 73 del post viejo quedó fuera por
instrucción: la forma de puntuar cambió el 23-sep y ese número ya no se reproduce. No se usó
ningún otro número propio, así que la pieza no envejece con el producto.

**Sin alarmismo, que acá era el riesgo real.** Un post sobre multas se desliza solo hacia el
miedo. Este va en la dirección contraria: los tres mitos que desarma son, los tres, cosas que
asustan más de lo que la ley justifica. Es coherente con el guardrail del brief ("nada de
alarmismo") y con el hecho de que el producto se vende por la fecha, no por el susto.

**El compartir personal no resume el post.** Agrega lo que la marca en plural no puede decir: que
Ramón tenía el dato malo. Es la convención del par página + perfil que ya funcionó el 21-sep, y
el momento incómodo es real y está documentado (el brief interno listaba las 72 horas como
requisito).

**Sin fecha de vigencia en el cuerpo.** El post no menciona el 1-dic-2026 en ninguna parte, así
que el proyecto de postergación (boletín 18.623-07, urgencia Suma renovada el 22-sep) no lo
puede dejar desactualizado. Fue deliberado. Si Ramón quiere anticiparse a un abogado que
pregunte, la línea corta está abajo, en "Opciones que quedan a criterio de Ramón".

**Chequeo de tono.** Sin guion largo. Sin transición de relleno. Sin superlativo. Sin urgencia
fabricada. Dos remates en todo el cuerpo ("no el requisito" y "no un reloj"), que es el tope que
el corpus fija en dos. Un solo uso de dos puntos en el cuerpo. Registro plural puro en la página,
singular puro en el compartir, sin mezcla.

---

## Opciones que quedan a criterio de Ramón

1. **Línea sobre la postergación, si la quiere.** No la metí porque el post no afirma ninguna
   fecha. Si un abogado pregunta en comentarios, esta responde sin comprometer nada:
   > Hay un proyecto en el Senado para mover la entrada en vigencia a diciembre de 2027
   > (boletín 18.623-07). Está en primer trámite y mientras no se apruebe, rige el 1 de
   > diciembre de 2026.
2. **Hashtags.** Sin poner, igual que el post de página del 21-sep. Si los quiere, calzan
   `#Ley21719 #ProtecciónDeDatos #Chile`.
3. **El cuarto mito, para otra pieza.** El artículo sexto transitorio dice que durante los
   primeros doce meses la Agencia **podrá** aplicar amonestación escrita a empresas de menor
   tamaño. Circula como "la primera infracción de una pyme es amonestación", y eso también es
   falso: es una facultad de la Agencia, no un derecho de la empresa, y está acotada a doce
   meses. **No entró en este post a propósito**, porque el brief interno lo afirma en la versión
   incorrecta y desmentirlo acá obligaba a explicar demasiado. Da para una pieza propia.
