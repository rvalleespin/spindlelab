...
- **Sitio (`persona-disenador-web`, Diego):** migración a **Astro** (`spindlelab-astro/`) completa — jerarquía de servicios reordenada, método nombrado "Método Spindle", PR #18 (mejoras incrementales) y PR #19 (blog + servicios + método + contacto portados, 14/14 URLs verificadas en `astro preview`, sitemap y JSON-LD intactos) ya fusionados. **Corrección 3-ago: el dominio real (spindlelab.cl) NO se ha apuntado todavía** — solo existe como preview (`spindlelab-v2.pages.dev`), lo dije como "en vivo" antes y era incorrecto. El commit de PR #19 lo deja explícito: "eso es un paso de go-live posterior a la verificación del troncal" — **es decir, va apuntar el dominio.** Falta: (a) revisar el sitio Astro en vivo/preview, (b) confirmar la tipografía del hero que no calza con Gabarito/Manrope del manual (¿deliberada?), (c) apuntar el dominio.
