...
- 2026-09-02 — **SpindleLab, sprint de dirección creativa con Ramón en vivo.** Concepto
  ...
  (idea de Ramón): `_reserva/sprint-concepto-01sep/kv-F-domino.png`. Lleva un **"desde"**
...
- **"Facebook está bloqueado" era diagnóstico falso.** `curl` a facebook.com devuelve 200
  ...
  `2026-09-septiembre/_reserva/sprint-concepto-01sep/competencia-adlib/hallazgos.md`.
  **Principio: antes de reportar un host como bloqueado, probarlo con `curl` y con OTRO
  host; si los dos caen, el problema es mi herramienta, no la política.** (1-sep)
- **Cambiar el fondo NO es rediseñar la pieza.** Entregué la misma ficha con otra textura
  ...
  cambió el papel, no el diseño. (1-sep)
...
- **La Ad Library dice qué SOSTIENEN, no qué rinde.** Se mide por antigüedad del ángulo y
  ...
  Ojo con "total active time <1 hr" en creatividades sueltas: rotan muchísimo. (1-sep)
- **Un relato visual mal armado se anula solo.** La v1 del circuito dejaba encendido el
  ...
  que la pieza no se contradiga a sí misma, no solo que se vea bien.** (1-sep)
- **Una foto generada trae elementos que contradicen el relato; se arreglan con el
  ...
  sacar del cuadro.** Lo cazó Ramón, no yo: yo miré el PNG y no lo vi. (1-sep)
- **Verificar la fecha REAL antes de fechar nada** (`date`), no deducirla del encargo. El
  encargo planificaba el sprint para el mié 2-sep; se corrió el mar **1-sep** y yo feché
  todo como 2-sep: reporte, encargos, memoria, commits y hasta el nombre de la carpeta.
  Peor: por eso dije "mañana" hablando de pasado mañana, y descuadré a Ramón sobre qué
  pieza salía qué día. **Una fecha mal puesta en un repo de coordinación desordena a todo
  el resto de las sesiones, no solo a mí.** (1-sep)
