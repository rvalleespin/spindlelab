...
**Correction, 10-sep-2026:** an earlier note here (1-sep) claimed a separate clone at `~/Projects/spindlelab` was "the good clone" and called this iCloud Drive copy corrupt. Verified 10-sep: `~/Projects/spindlelab` does not exist on the machine, and this iCloud copy passes `git fsck --connectivity-only` clean (no missing objects, no broken links — only normal dangling objects). Whatever prompted the original warning either never happened here or was already fixed. **Work directly in this iCloud Drive copy** unless a future session finds real evidence of corruption — and if it does, it should replace this note with what it found, not just repeat the old one.
